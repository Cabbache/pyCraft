...
def getClosest(coor):
	sy = math.floor((coor - 64.0)/128.0) + (1 if coor < 0 else 0)
	for x in xrange(0, 600):
		sy += 1 if coor > 0 else -1
		for n in xrange(-300, 300):
			for z in xrange(9, 30):
				if 128*sy == 90 + (n*158) + z:
					closest = 128*(sy) + 64
					return int(closest)
	return "No solution"
...
def drawImage(length, width, cont, intell):
	...
	global exit
	if not(cont):
		if intell and pic.size[0] == 128 and pic.size[1] == 128:
			get128()
	...
	for sx in range(start[0], width):
		for sz in range(start[1] if first else 0, length):
			...
			while len(zeroQueue) > 0:
				...
				itemID = rgbToBlockID(im[zero[0] - basex, zero[2] - basez])
				getItem(36, itemID)
			...
			itemID = rgbToBlockID(im[sx, colorz])
			...
			for j in range(len(queue)):
				try:
					...
					if time.time() - prePlacement[3] > 5:#if 5 seconds elasped and no confirmation
						zero = [prePlacement[0], prePlacement[1], prePlacement[2]]
						#transfer block from queue to the zeroQueue
...
def update_change(pkt):#this is called when a block is placed or destroyed
	x = pkt.location.x
	y = pkt.location.y
	z = pkt.location.z
	if pkt.block_state_id == 0 and distFrom(x,y,z) < 30:
		coords = [x,y,z]
...
def UI(abc):
	...
	while not(exit):
		...
		if action == "ch":
			...
			if len(command) == 1:
				drawImage(pic.size[0], pic.size[1], False, False)
				continue
			if command[1] == "cont" and os.path.exists("Last.txt"):
				drawImage(pic.size[0], pic.size[1], True, False)
			elif command[1] == "intel":
				drawImage(pic.size[0], pic.size[1], False, True)
		elif action == "discon":
			conn.disconnect()
			posUpdate.stop()
		elif action == "connect":
...
im = pic.load()
print("loaded image (" + str(pic.size[0]) + "x" + str(pic.size[1]) + ")")
