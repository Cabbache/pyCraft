...
from minecraft.networking.types import (
	...
	RelativeHand, BlockFace, Short, UnsignedByte
)
...
class ClickWindowPacket(Packet):
	...
	@staticmethod
	def get_id(context):
		return 0x08 if context.protocol_version >= 389 else \
			   0x06 if context.protocol_version >= 343 else \
			   0x07 if context.protocol_version >= 336 else \
			   0x08 if context.protocol_version >= 318 else \
			   0x07 if context.protocol_version >= 80 else \
			   0x06 if context.protocol_version >= 67 else \
			   0x0F if context.protocol_version >= 49 else \
			   0x0E
...
class UseItemPacket(Packet):
	...
	def get_id(context):
		return 0x2A if context.protocol_version >= 389 else \
			   ...
			   0x1D if context.protocol_version >= 80 else \
			   0x1B if context.protocol_version >= 77 else \
			   0x1A if context.protocol_version >= 67 else \
			   0x08 if context.protocol_version >= 49 else \
			   0x08
...
class CreativeInventoryActionPacket(Packet):
	@staticmethod
	def get_id(context):
		return 0x24 if context.protocol_version >= 389 else \
               0x1B if context.protocol_version >= 318 else \
			   0x18 if context.protocol_version >= 80 else \
			   0x16 if context.protocol_version >= 77 else \
			   0x15 if context.protocol_version >= 67 else \
			   0x11 if context.protocol_version >= 49 else \
			   0x10

	packet_name = 'creative inventory action'
